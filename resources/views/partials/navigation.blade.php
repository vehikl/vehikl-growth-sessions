<nav class="bg-vehikl-primary p-8">
    <div class="flex items-center justify-between flex-wrap">
        ...
        <div
                class="w-full hidden text-xl lg:flex justify-end lg:items-center lg:w-auto text-center "
                id="nav-links">
            <div class="text-xl gap-4 justify-center items-center flex flex-col lg:flex-row text-white">
                <a href="{{route('about')}}"
                   class="hover:text-vehikl-secondary font-display transition-colors">About
                </a>
                @auth

                    @if(\Illuminate\Support\Facades\Route::is('statistics.index'))
                        <a href="{{route('home')}}" class="hover:text-vehikl-secondary transition-colors">
                            Board
                        </a>
                    @else
                        <a href="{{route('statistics.index')}}"
                           class="hover:text-vehikl-secondary transition-colors">
                            Statistics
                        </a>
                    @endif
                    <a href="{{ route('logout') }}"
                       onclick="event.preventDefault(); document.getElementById('logout-form').submit();"
                       class="flex items-center hover:text-vehikl-secondary tracking-wider transition-colors">
                        <v-avatar class="mr-2"
                ...
                @guest
                    <a href="{{route('oauth.login.redirect', ['driver' => 'github'])}}"
                       class="flex items-center hover:text-vehikl-secondary transition-colors">
                        <i class="fa fa-github text-3xl mr-2" aria-hidden="true"></i> Login
                    </a>
                    @if(config('services.google.client_id'))
                        <a href="{{route('oauth.login.redirect', ['driver' => 'google'])}}"
                           class="flex items-center hover:text-vehikl-secondary transition-colors">
                            <i class="fa fa-google text-3xl mr-2" aria-hidden="true"></i> Login
                        </a>
                    @endif
                @endguest
