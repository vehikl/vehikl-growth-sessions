...
const { watchGrowthSession, leaveGrowthSession, joinGrowthSession, growthSession, isProcessing } = useGrowthSession(props.growthSessionJson);
    ...
    <div class="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8">
        ...
        <div class="grid grid-cols-1 gap-12 md:grid-cols-2">
            <div>
                ...
                <div class="mb-2 flex justify-between text-slate-500">
                    ...
                    <div class="attendees-count flex items-center">
                        ...
                        <span v-if="growthSession.attendee_limit" class="attendee-limit pl-1">of {{ growthSession.attendee_limit }}</span>
                    </div>
                </div>
                ...
                <pre
                    class="topic break-words-fixed mb-4 inline-block max-h-64 overflow-x-hidden overflow-y-auto text-left font-sans leading-relaxed tracking-wide whitespace-pre-wrap text-slate-600"
            ...
            <div class="row-span-2">
                ...
                <div class="mb-4">
                    <a v-if="growthSession.discord_channel_id" class="location-icon" target="_blank" :href="discordChannelUrl">
                ...
                <div v-if="growthSession.anydesk" class="mb-4">
                    <h3 class="text-lg font-semibold tracking-widest text-slate-600 uppercase">AnyDesk</h3>
                ...
                <div class="mb-4" v-if="growthSession.attendee_limit">
                    <h3 class="text-lg font-semibold tracking-widest text-slate-600 uppercase">Attendee Limit</h3>
                    <span class="attendee_limit">{{ growthSession.attendee_limit }}</span>
                </div>

                <h3 class="text-lg font-semibold tracking-widest text-slate-600 uppercase">Attendees</h3>
                <ul class="mb-4">
                    <li v-for="attendee in growthSession.attendees" :key="attendee.id" class="py-2">
                        <a ref="attendee" :href="attendee.githubURL" class="flex items-center">
                            ...
                            <p v-else class="ml-2 text-sm font-bold tracking-wider text-slate-600 uppercase">{{ attendee.name }}</p>
                        </a>
                    </li>
                </ul>

                <h3 v-if="growthSession.watchers.length" class="text-lg font-semibold tracking-widest text-slate-600 uppercase">Watchers</h3>
                <ul>
                    <li v-for="watcher in growthSession.watchers" :key="watcher.id">
                        <a ref="attendee" :href="watcher.githubURL" class="mb-4 flex items-center">
                            ...
                            <p v-else class="ml-2 text-sm font-bold tracking-wider text-slate-600 uppercase">{{ watcher.name }}</p>
                        </a>
                    </li>
                </ul>
            </div>
