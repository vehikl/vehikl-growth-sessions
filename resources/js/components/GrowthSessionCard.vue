...
<template>
    <a class="block border-l-4 p-4 pt-2 mx-2 cursor-pointer overflow-visible mb-3 transform transition-transform duration-150 shadow-md bg-white"
       :class="growthSession.is_public ? 'border-emerald-900' : 'border-vehikl-orange'"
       ...
       <div v-if="growthSession.title || isDraggable"
        ...
        <div class="flex items-center justify-between flex-1 mb-2 text-emerald-900">
       ...
       <h3 class="font-light text-4xl font-extrabold text-emerald-900 text-left mb-3"
        ...
        <div class="text-emerald-600 text-left mb-4 break-all">
            <i class="fa fa-compass text-xl mr-2" aria-hidden="true"></i>
            <location-renderer :locationString="growthSession.location"/>
        </div>

        <div v-if="growthSession.anydesk && user" class="text-emerald-700 text-left mb-4 break-all">
