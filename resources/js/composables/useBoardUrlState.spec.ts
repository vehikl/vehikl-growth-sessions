...
describe('useBoardUrlState', () => {
    ...
    describe('shiftDateBy', () => {
        ...
        it('drops the open session in the same history entry, since it belongs to the week left behind', () => {
            const win = fakeWindow('?session=42');
            const { sessionId, syncFromUrl, shiftDateBy } = useBoardUrlState(win);
