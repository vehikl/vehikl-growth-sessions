...
export function useBoardUrlState(win: BoardUrlWindow = typeof window === 'undefined' ? noBrowserWindow : window) {
    ...
    /**
     * Move the board's week and record the move in the url. Any open session belongs to the week
     * being left behind — keeping it in the url would hand out a link that opens no drawer — so it
     * is dropped in the same history entry, costing the visitor one press of Back rather than two.
     */
